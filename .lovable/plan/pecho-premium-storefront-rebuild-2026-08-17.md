...
- Final check across 1440 / 1280 / 1024 / 390 / 375 for overflow, stretched media, Georgian text, variant switching, cart flow and console errors.  
  ...
  1. STORE LOCATIONS / ADMIN
  &nbsp;
  Do not treat browser localStorage as the final persistence layer for admin-managed store locations.
  The admin location manager should be architected so locations can later be stored persistently in a real backend/database without changing the customer-facing map UI.
  For now, if the current project has no backend/database available, you may use the existing coded locations as the default source and localStorage only as a temporary development fallback. Clearly isolate this behind a small location data/service layer so it can be replaced by persistent storage later.
  The customer-facing map must remain strictly read-only. Customers must never see admin controls.
  &nbsp;
  2. 2+1 BUNDLE
  The 2+1 promotion must remain a real purchasable product/bundle, not just a promotional image or banner.
  It must have:
  - its own product card
  ...
  The announcement bar must link directly to this 2+1 product page.
  Do not break the existing cart, checkout, order submission, or admin order flow.
  Everything else in the proposed Phase 1 → Phase 2 plan is approved.